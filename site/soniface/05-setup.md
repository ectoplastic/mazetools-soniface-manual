...
## Setup: Pattern, Audio, Video, MIDI
...
The default settings of Pitch (1), Tempo (120 bpm) and Volume (0.5) are set.

## Rotate Spatial

Turn the spatial behavior for the sake of a sound system opposite position.
...
Zoom in/ out the whole Soniface user Interface.
...
Fix the Volume for all Patterns of the current Set.

## Fix Tempo

Fix the Tempo for all Patterns of the current Set.

## Fix Pitch

Fix the Pitch Shift for all Patterns of the current Set.

## Time Reset

Sets the Soniface Player loop back to the start position. This may be necessary for Beat Sync.
...
Activates Hi-Res mode for all Samplers and Rhythm instruments.
...
Instrument Tuning before Soniface 3.5 update.

## Camera usage for controlling sound through movements

Mazetools Soniface uses the device camera to capture the body and movements to control the Maze controllers.

## Microphone use for voice and noise recording

The device microphone is used to record voice, music or noise and use it directly in a composition.

## Network usage for tempo matching with Ableton Link

Ableton Link is a technology that synchronizes tempo with other Link apps over a local network.

## Inter-App Audio & Audiobus On/ Off
...
On/off switch to keep Soniface sound running in the background, turning off all graphics processes.

## Simple design

Toggle between mobile and desktop user Interface. The design becomes active only after restarting the app.

## UI Color Mode

Dynamic customization of the user Interface color scheme.
...
Initialization when an external monitor is connected.
...
The notes of the Gridsynth are output via MIDI Out, e.g. to control another synthesizer with it.
...
The head position is reset. Only possible with HFTF capable headphones (e.g. Airpods with 3D Audio).

## Refresh MIDI Ports

The available MIDI ports are updated.

## Reset MIDI

The link to a MIDI port is removed.

## Master Sound Settings

In the master bus you can filter the whole audio signal. The settings can be changed pattern by pattern.

## Equalizer

The EQ contains the following filters: Lowpass, Highpass, Bandpass, Peak, Notch, Lowshelf, Highshelf. It can be changed pattern by pattern.

## Highcut Filter On/ Off

A filter that passes signals with a frequency higher than a selected cutoff frequency

## Lowshelf Filter On/ Off
...
A peak EQ filter makes a peak or a dip in the frequency response.

## Highshelf Filter On/ Off
...
The filter curve can be defined narrow or wide.

## Peak/ Notch Frequency
...
A high-Q filter will have a narrow passband and a low-Q filter will have a wide passband.

## Shelf Frequency & Level
